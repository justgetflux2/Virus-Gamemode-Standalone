...
local models = {
	normal = Model("models/player/Group03/male_04.mdl"),
...
function GiveWeapons(pl)
		pl:GiveAmmo( 42,	"Pistol", 		true )
		pl:GiveAmmo( 30,	"SMG1", 		true )
		pl:GiveAmmo( 64,	"Buckshot", 	true )
-- this is a lazy way to just give random weapon sets
if (currentRound.number == 1) then
		pl:Give( "weapon_dsilen" )
        pl:Give( "weapon_9mm" )
		pl:Give( "weapon_thompson" )
		pl:Give( "weapon_doublebarrel" )
        pl:Give( "weapon_sniper" )
    elseif (currentRound.number == 2) then
        pl:StripWeapon( "weapon_doublebarrel" )
		pl:Give( "weapon_sonicshotgun" )
        pl:Give( "weapon_thompson" )
        pl:Give( "weapon_sniper" )
        pl:Give( "weapon_9mm" )
        pl:Give( "weapon_flakgun" )
    elseif (currentRound.number == 3) then
        pl:StripWeapon( "weapon_flakgun" )
		pl:StripWeapon( "weapon_thompson" )
        pl:StripWeapon( "weapon_sonicshotgun" )
		pl:Give( "weapon_dsilen" )
        pl:Give( "weapon_doublebarrel" )
        pl:Give( "weapon_9mm" )
        pl:Give( "weapon_sniper" )
        pl:Give( "weapon_rcp120")
 elseif (currentRound.number == 4) then
        pl:StripWeapon( "weapon_doublebarrel" )
        pl:StripWeapon( "weapon_rcp120" )
		pl:Give( "weapon_sonicshotgun" )
        pl:Give( "weapon_thompson" )
        pl:Give( "weapon_sniper" )
        pl:Give( "weapon_9mm" )
        pl:Give( "weapon_flakgun" )
 elseif (currentRound.number == 5) then
        pl:Give( "weapon_sonicshotgun" )
 elseif (currentRound.number == 6) then
        pl:Give( "weapon_sonicshotgun" )
        pl:Give( "weapon_thompson" )
        pl:Give( "weapon_sniper" )
        pl:Give( "weapon_9mm" )
 elseif (currentRound.number == 7) then
        pl:StripWeapon( "weapon_dsilen" )
		pl:StripWeapon( "weapon_smg1" )
		pl:Give( "weapon_doublebarrel" )
 elseif (currentRound.number == 8) then
        pl:Give( "weapon_dsilen" )
		pl:Give( "weapon_thompson" )
		pl:Give( "weapon_doublebarrel" )
        pl:Give( "weapon_sniper" )
...
local function checkRoundState()
	if currentRound.noOfInfected == 0 then roundFinish(true) end --save fail in case the selected player ragequits =/ Stupid kleiners!
...
function roundFinish(forced) // TODO: Remove or revise forced mechanic, unless if amount of players dips below a threshold we need to force end the game at some point.
	...
	net.Broadcast()
    RunConsoleCommand("stopsound") -- I dont want to use run console command so fuck..
   	umsg.Start( "SurvivorsWin")
	umsg.End()
...
--[[local function PlayMusic()
   if setupPhase() then
      BroadcastLua('surface.PlaySound("virus.setup")')

   elseif roundStart() then
      BroadcastLua('surface.PlaySound("virus.round")')

 --  elseif wintype == WIN_TIMELIMIT then
     -- BroadcastLua('surface.PlaySound("yourmusic.whatever")')
   end
end
hook.Add("EndRound", "MyMusic", PlayMusic)]]
