...
local function DrawNotice(k, v, i)
	...
	local distY = ideal_y - v.y
	v.vely = v.vely + distY * spd * 1
	if (math.abs(distY) < 2 && math.abs(v.vely) < 0.1) then v.vely = 0 end
	local distX = ideal_x - v.x
	v.velx = v.velx + distX * spd * 1
	if (math.abs(distX) < 2 && math.abs(v.velx) < 0.1) then v.velx = 0 end
